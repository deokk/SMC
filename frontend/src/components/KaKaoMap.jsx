...
export default function KakaoMap({
  ...
  const END_MARKER_IMAGE_SRC =
    'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30"><circle cx="15" cy="15" r="12" fill="%23DC3545" stroke="%23FFFFFF" stroke-width="2"/><text x="15" y="19" font-family="Arial" font-size="10" fill="%23FFFFFF" text-anchor="middle" font-weight="bold">도착</text></svg>';
  const MY_LOCATION_MARKER_IMAGE_SRC =
    'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20"><circle cx="10" cy="10" r="8" fill="%2300A8FF" stroke="%23FFFFFF" stroke-width="2"/></svg>';
  const FRIEND_MARKER_IMAGE_SRC =
  ...
  useEffect(() => {
    ...
    stationsToDraw.forEach((station) => {
      ...
      const marker = new window.kakao.maps.Marker({
        ...
        title: station.station_name || station.station_display_name,
      });
      marker.setMap(map);
      markersRef.current.push(marker);

      // 개수 오버레이는 항상 표시
      const currentBikes = isPredictionMode
        ? predictionData[station.station_id] ?? "?"
        : station.available_bikes;
      const overlayContent = document.createElement("div");
      overlayContent.className = isPredictionMode
        ? "overlay-content prediction"
        : "overlay-content";
  ...
  useEffect(() => {
    ...
    const friendMarkerImage = new window.kakao.maps.MarkerImage(
      FRIEND_MARKER_IMAGE_SRC,
    ...
    if (!initialFriendBoundsSetRef.current && friendLocations.length > 0) {
      ...
      friendLocations.forEach((friend) => {
        if (friend.latitude !== undefined && friend.longitude !== undefined) {
          bounds.extend(
            new window.kakao.maps.LatLng(friend.latitude, friend.longitude)
          );
        }
      });
      if (!bounds.isEmpty()) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      selectedRoute.steps.forEach((step) => {
        ...
        if (step.type === "SUBWAY")
          lineColor =
            SUBWAY_LINE_COLORS[step.name] || SUBWAY_LINE_COLORS.default;
      ...
      const firstStep = selectedRoute.steps.find(
        (s) => s.polyline && s.polyline.length > 0
      );
      ...
      if (firstStep && lastStep?.polyline?.length > 0) {
        const startCoord = new window.kakao.maps.LatLng(
          firstStep.polyline[0][0],
          firstStep.polyline[0][1]
        );
        const endCoord = new window.kakao.maps.LatLng(
          lastStep.polyline[lastStep.polyline.length - 1][0],
          lastStep.polyline[lastStep.polyline.length - 1][1]
        );

        const startMarkerImg = new window.kakao.maps.MarkerImage(
          START_MARKER_IMAGE_SRC,
          new window.kakao.maps.Size(30, 30),
          { offset: new window.kakao.maps.Point(15, 30) }
        );
        const endMarkerImg = new window.kakao.maps.MarkerImage(
          END_MARKER_IMAGE_SRC,
          new window.kakao.maps.Size(30, 30),
          { offset: new window.kakao.maps.Point(15, 30) }
        );
  ...
  useEffect(() => {
    ...
    if (riderLocation) {
      const riderPosition = new window.kakao.maps.LatLng(
      ...
      if (!riderMarkerRef.current) {
        ...
        if (!riderMarkerRef.current.getMap()) {
          // 마커가 지도에 없으면 다시 추가
